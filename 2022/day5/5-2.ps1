...
foreach($instruction in $data){
    if($instruction -like "move*"){
        ...
        #foreach($i in 0..($SourceAmt -1)){

            $container[$destStack] = ($container[$sourceStack][0..($sourceAmt -1 )] -join('')) + $container[$destStack]             
            $container[$sourceStack] = $container[$sourceStack].substring(($sourceAmt))
